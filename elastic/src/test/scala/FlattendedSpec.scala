...
class FlattenedSpec extends FlatSpec with Matchers {
  ...
  def point =
    Datapoint[Any](Key[Double](java.util.UUID.randomUUID.toString, Units.Count, "some description",
      Map(AttributeKeys.source -> "http://10.0.10.10/stream/now", AttributeKeys.kind -> "gauge")
    ), 3.14)

  def point2 =
    Datapoint[Any](Key[Stats](java.util.UUID.randomUUID.toString, Units.Count, "some description",
      Map(AttributeKeys.source -> "http://10.0.10.10/stream/now", AttributeKeys.kind -> "gauge")
    ), Stats(3.14))
