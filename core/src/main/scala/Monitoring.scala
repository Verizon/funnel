...
trait Monitoring {
  ...
  def processMirroringEvents(
    ...
    val active    = signalOf[Set[URI]](Set())(S)
  ...
  def decay(f: Key[Any] => Boolean)(e: Event): Task[Unit] = Task.delay {
    ...
    val alive = signalOf[Unit](())(Strategy.Sequential); alive.set(()).run
...
object Monitoring {
  ...
  def instance(implicit ES: ExecutorService = defaultPool,
    ...
    val P = Process
    val keys_ = signalOf[Set[Key[Any]]](Set())(S)
  ...
  private[funnel] def bufferedSignal[I,O](
      ...
      (I => Unit, Signal[O]) = {
    val signal = scalaz.stream.async.signalUnset[O](Strategy.Executor(ES))
