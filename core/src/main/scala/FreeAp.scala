...
// TAKEN VERBATIM FROM Scalaz series 7.2.x
// At the time of writing there is no suitable release of Scalaz that
// includes this data type, so it is reproduced here for the time being.
...
object FreeAp {
  ...
  private case class Pure[F[_],A](a: A) extends FreeAp[F,A]
