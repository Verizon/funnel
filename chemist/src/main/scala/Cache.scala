...
class GuavaCacheBuilder[K, V] extends CacheBuilder[K, V] {
  ...
  def maximumSize(n: Option[Int]):GuavaCacheBuilder[K,V] = n map {v => maximumSize(v)} getOrElse(this)
  def concurrencyLevel(n: Option[Int]):GuavaCacheBuilder[K,V] =  n map {v => concurrencyLevel(v)} getOrElse(this)
  def expireAfterWrite(duration: Option[Duration]):GuavaCacheBuilder[K,V] = duration map {v => expireAfterWrite(v)} getOrElse(this)
  def expireAfterAccess(duration: Option[Duration]):GuavaCacheBuilder[K,V]  = duration map {v => expireAfterAccess(v)} getOrElse(this)
}
